...
const ChatInterface: React.FC = () => {
  ...
  const [sessions, setSessions] = useState([
    { id: 1, name: "Anxiety before presentation", date: new Date() },
    {
      id: 2,
  ...
  const handleNewSession = () => {
    clearChat();
    setSessions([
      { id: Date.now(), name: "New Session", date: new Date() },
      ...sessions,
    ]);
  };
  ...
  return (
    <div className="flex h-screen bg-surface-900 text-gray-200 overflow-hidden font-sans">
      {/* Sidebar */}
      <div
        className={`transition-all duration-300 flex flex-col bg-[#1A1A1A] border-r border-[#2A2A2A] ${sidebarOpen ? "w-[260px] opacity-100" : "w-0 opacity-0 overflow-hidden"}`}
      >
        {/* Sidebar Header */}
            ...
            {showNewFolderInput && (
              <div className="mb-3 flex gap-2">
                <input
                  ...
                  className="flex-1 text-sm bg-[#252525] text-gray-200 rounded px-2 py-1 outline-none focus:ring-1 focus:ring-accent-500"
                />
                <button
                  onClick={handleNewFolder}
                  className="px-2 py-1 bg-accent-500 hover:bg-accent-600 text-white rounded text-xs font-medium transition-colors"
                >
                  Add
                </button>
              </div>
        ...
        <div className="p-4 border-t border-[#2A2A2A]">
          <button
            onClick={handleNewSession}
            className="w-full flex items-center justify-center gap-2 bg-accent-500 hover:bg-accent-600 text-white py-2.5 rounded-xl text-sm font-semibold transition-all active:scale-95"
          >
        ...
        <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center z-10 bg-gradient-to-b from-[#0E0E0E] to-transparent pointer-events-none">
          <div className="flex items-center gap-4 pointer-events-auto">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-[#142A1E] text-accent-400 border border-accent-500/20">
                v2.0
              </span>
            </div>
          </div>
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center w-full max-w-3xl mt-10">
              ...
              <p className="text-gray-400 mb-10 text-center">
                I provide a safe, anonymous space. Select support tools below or
                just start typing.
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 w-full mb-8">
                {(
                  Object.keys(featureDetails) as Array<keyof typeof features>
                ).map((key) => (
                  <div
                    key={key}
                    onClick={() => {
                  ...
                  >
                    ...
                    <h4
                      className={`text-sm font-medium mb-1 ${features[key] ? "text-accent-400" : "text-gray-200"}`}
                    >
              ...
              <div className="flex flex-wrap items-center justify-center gap-2">
                <span
                  className={`px-4 py-1.5 rounded-full text-xs font-medium cursor-pointer ${activeFeatures.length === 0 ? "bg-[#3A3A3A] text-white" : "text-gray-400 hover:bg-[#1A1A1A]"}`}
                >
                  Just talk
                </span>
                {Object.keys(featureDetails).map(
                  (key) =>
                    features[key as keyof typeof features] && (
                      <span
                        key={key}
                        className="px-4 py-1.5 rounded-full text-xs font-medium cursor-pointer bg-[#142A1E] text-accent-400 border border-accent-500/20"
                      >
                        {featureDetails[key].title}
                      </span>
                    ),
          ...
          <div className="max-w-3xl mx-auto w-full">
            ...
            <div className="text-center mt-3">
              <span className="text-[10px] text-gray-500">
                This AI assistant can make mistakes. Please consider verifying
